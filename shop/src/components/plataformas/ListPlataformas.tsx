...
const ListPlataformas: React.FC<{}> = () => {
  ...
  const isProvider = me?.permissions?.some(permission => permission.name === "provider");


  const handleSuscription = (plataforma: Plataforma) => {

    const prce = me && isProvider ? plataforma.provider_price : plataforma.public_price
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        createBanner({ variables: { plataforma_id: plataforma.id, user_id: me!.id } }, {
        ...
        <Card>
          {plataformas.map((plataforma, i) => {
            return (
              <SwiperSlide key={i}>
                <div
                  className={`w-full h-48 rounded-xl flex justify-center items-center relative ${plataforma.type === 'completa' ? 'bg-brand' : 'bg-blue-500'
                    } border-2 ${plataforma.type === 'completa' ? 'border-brand-dark' : 'border-blue-400'
                    } `}
                >
                  <Image
                  ...
                  <div className="absolute inset-0 flex flex-col justify-center items-center bg-black bg-opacity-50 rounded-xl">
                    <span className="absolute top-1 right-3 uppercase">{plataforma.type}</span>
                    <span className="font-bold text-white md:text-2xl mb-2 uppercase text-xl flex flex-col text-center">
                      {plataforma.name} <span className="text-xs">({plataforma.count_avaliable} disponibles)</span>
                    </span>
                    <p className="text-xl text-white">

                      COP ${me && isProvider ? plataforma.provider_price : plataforma.public_price}
                    ...
                    </p>
                    {
                      me ? (
                        ...
                        !isSuperAdmin && (
                          <button onClick={() => handleSuscription(plataforma)} className="cursor-pointer p-3 mt-4 uppercase text-sm font-semibold text-white bg-[#1A1A1A] rounded hover:bg-[#333333] transition ease-in-out hover:scale-105 duration-300">
                            Suscribirme
                          </button>
                        )

                      ) :
                        <AnchorLink href={routes.login} className="cursor-pointer p-3 mt-4 uppercase text-sm font-semibold text-white bg-[#1A1A1A] rounded hover:bg-[#333333] transition ease-in-out hover:scale-105 duration-300">
                          Suscribirme
                        </AnchorLink>
                    }
                  </div>
