...
export function getUrlLinks(
  ...
  try {
    ...
    const processLink = (link: SidebarLink): void => {
      const isMatch =
        (link.href && link.href.includes(searchTextLower)) ||
        link.label.includes(searchTextLower);
...
export function formatOwnerLinks(
  links: OwnerLink[],
  shop: string
): SidebarLink[] {
...
function makeStringReadable(inputString: string): string {
  ...
  const firstWord: string = secondElement.replace(/\b\w/g, (match: string) =>
    match.toUpperCase()
  );
